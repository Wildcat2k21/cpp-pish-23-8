...
int main(int argc, char** argv) {
    ...
    if(opt.sum) {
        std::cout << "Сумма: " << std::accumulate(values.begin(), values.end(), 0) << std::endl;
    } else if(opt.mult) {
        std::cout << "Произведение: " << std::accumulate(values.begin(), values.end(), 1, std::multiplies<int>()) << std::endl;
    } else {
